...
public interface StatsRepository extends JpaRepository<Stats, Integer> {
    ...
    Collection<GetStatsDto> findAllStatsWithUris(LocalDateTime start, LocalDateTime end, List<String> uris);

    @Query("SELECT S.app AS app, S.uri AS uri, COUNT (distinct S.ip) AS hits " +
            "FROM Stats AS S WHERE S.timeStamp BETWEEN :start AND :end " +
            "AND S.uri in :uris GROUP BY S.app, S.uri order by hits DESC ")
    Collection<GetStatsDto> findUniqueStatsWithUris(LocalDateTime start, LocalDateTime end, List<String> uris);
